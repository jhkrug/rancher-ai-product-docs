...
+
--
The **SUSE-Observability** agent helps you gather metric, traces and logs from your SUSE-Observability installation and clusters.

**Pre-requisites:**

1. link:https://documentation.suse.com/cloudnative/suse-observability/latest/en/advanced/ai-assistant.html[SUSE-Observability] deployed with MCP server enabled
...
1.  **Generate a token:** Visit the link:https://documentation.suse.com/cloudnative/suse-observability/latest/en/use/ai-assistant/mcp-server.html[SUSE Observability MCP] page to generate the token and get the `mcp` endpoint.
2. Create a secret in the **local** cluster using the token from step 1.
[source,bash]
----
...
----
3.  **Navigate to Settings:** Go to **Global Settings** > **AI Assistant**.
4.  **Add Agent:** Click **Add AI Agent** and input the following:
...
spec:
  ...
  description: "SUSE Observability Agent part of Liz's crew. It can access advanced
    ...
    telemetry. \nExample of prompts: \n- Give me a top 5 of resource usage in namespace
    my-app\n- What are the dependencies of the checkout pod in the sock-shop namespace?\n-
...
+
--
The **SUSE-Security** agent helps you scan your environement. The agent call into SBOMscanner to list registries, scan jobs and workloads, inspect specific CVEs, follow scan progress, manage VEX hubs, and (when you want it to) create, update, or delete the corresponding resources for you.
...
1. Create a secret in the **local** cluster
...
----
2. Then configure programmatically the **AIAgentConfig** with `caBundleRef`
